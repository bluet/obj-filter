...
test('filter.exist should contain: runtime, connectionState, powerState, bootTime, obj jj, kk', function (t) {
	var result_exist = filter.exist(template, data_success);
	t.deepEqual(result_exist, exam_success);
	t.end();
});

test('filter.exist should fail', function (t) {
	var result_exist_fail = filter.exist(template, data_fail);
	t.false(result_exist_fail);
	t.end();
});